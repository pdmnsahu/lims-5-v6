// pdfGenerator.js
// Images are resized via Cloudinary on-the-fly transformation URLs before fetching.
// No sharp dependency. Cloudinary CDN serves pre-compressed versions.
// A 3MB Parr photo becomes ~60KB — PDFs scroll smoothly.
...
// Inserts Cloudinary transformation params into the URL so their CDN
// serves a pre-compressed, pre-resized version. Works on any stored Cloudinary URL.
function cloudinaryResized(url, width, quality) {
  if (!url) return '';
  if (!url.includes('cloudinary.com')) return url;
  return url.replace('/upload/', `/upload/w_${width},q_${quality},f_jpg,c_limit/`);
}
...
async function inlineImages({ tests, settings }) {
  ...
  const [logo, acc, stamp, sig, parr] = await Promise.all([
    toDataURI(cloudinaryResized(settings.logo_url,          300, 85)),
    toDataURI(cloudinaryResized(settings.accreditation_url, 300, 85)),
    toDataURI(cloudinaryResized(settings.stamp_url,         300, 85)),
    toDataURI(cloudinaryResized(settings.signature_url,     400, 85)),
    toDataURI(cloudinaryResized(gcvTest?.image_url,         800, 70)),
  ]);
